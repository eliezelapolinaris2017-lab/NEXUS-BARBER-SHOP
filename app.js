// app.js — Nexus Barber Shop (Móvil)
...
let state = {
  ...
  pdfHeaderText: "",
  pdfFooterText: "",
  footerText: "© 2025 Nexus Barber Shop — Sistema móvil",
  theme: "dark",
  tickets: [],
  barbers: [],
  agenda: [],
...
// Cita que se está editando (por id). Por ahora solo borramos.
...
function renderBranding() {
  ...
  footerTextInput.value = state.footerText || "© 2025 Nexus Barber Shop — Sistema móvil";
  footerTextSpan.textContent = state.footerText || "© 2025 Nexus Barber Shop — Sistema móvil";
...
async function saveTicket() {
  ...
  try {
    ...
    formMessage.textContent = currentEditingNumber
      ...
      : "Ticket guardado y sincronizado.";
...
async function saveAgendaItem() {
  ...
  // NO PISAR HORAS (mismo día + misma hora + mismo barbero)
  const clash = state.agenda.find((c) => {
    if (!c.date || !c.time || !c.barber) return false;
    if (currentAgendaId && c.id === currentAgendaId) return false;
    return (
      c.date === date &&
      c.time === time &&
      c.barber === barber &&
  ...
  if (clash) {
    calendarWarning.textContent =
      "Ya existe una cita para este barbero en ese día y hora.";
    return;
  }
